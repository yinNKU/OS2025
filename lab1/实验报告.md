...
- **功能**：将当前 PC 值的高 20 位加载到 t0 寄存器
- **执行后**：`t0 = 0x1000`
- **目的**：获取代码基地址，用于后续的位置无关寻址

**指令2：`0x1004: addi a1, t0, 32`**
- **功能**：计算 `t0 + 32`，结果存入 `a1`
- **执行后**：`a1 = 0x1020`
- **目的**：准备函数参数，`a1` 可能指向设备树（Device Tree）的地址

**指令3：`0x1008: csrr a0, mhartid`**
- **功能**：读取 `mhartid` CSR（控制状态寄存器）到 `a0`
- **执行后**：`a0 = 0`（当前硬件线程 ID）
- **目的**：获取 CPU 核心 ID，用于多核启动（确定当前是哪个核心）

**指令4：`0x100c: ld t0, 24(t0)`**
- **功能**：从内存地址 `0x1018` (0x1000 + 24) 加载 8 字节数据到 `t0`
- **执行后**：`t0 = 0x80000000`（OpenSBI 的起始地址）
- **目的**：读取 OpenSBI 的入口地址

**指令5：`0x1010: jr t0`**
- **功能**：无条件跳转到 `t0` 寄存器指向的地址
- **执行后**：`PC = 0x80000000`
- **目的**：将控制权转移给 OpenSBI 固件
