...
- 在 `kern/trap/trap.c` 的 `exception_handler()` 中新增 `CAUSE_ILLEGAL_INSTRUCTION` 与 `CAUSE_BREAKPOINT` 两个分支。
- 每个分支都会：
	- 读取当前 `tf->epc` 保存的触发指令地址。
	- 通过 `cprintf` 输出异常来源，例如 `Illegal instruction caught at 0x...`、`Exception type: Illegal instruction`，以及 `ebreak caught at 0x...`、`Exception type: breakpoint`。
	- 将 `tf->epc` 前移 4 字节，让 `sret` 返回后跳过出错指令，避免重复陷入。
- 以上处理复用了定时器分发流程创建的 `trapframe`，无需额外保存寄存器，充分体现“陷入统一入口 +分发”结构的便利性。

## 调试与验证方法

- 在 `kern/init/init.c` 的 `intr_enable();` 之后临时加入两条内联汇编：
	```c
	asm volatile(".word 0xffffffff");  // 触发非法指令
	asm volatile("ebreak");            // 触发断点异常
	```
	这样能够在内核打开中断后立即制造一条非法指令和一条断点指令。
- 运行 `make qemu` 后，QEMU 控制台会按顺序打印：
	- `Illegal instruction caught at 0x...`
	- `Exception type: Illegal instruction`
	- `ebreak caught at 0x...`
	- `Exception type: breakpoint`
	地址字段对应 `tf->epc`，验证我们正确捕获并打印上下文。
- 若需单步排查，可改用 `make debug` 搭配 `make gdb`，在 GDB 中检查 `tf->epc`、`tf->cause` 等字段，确认 `epc` 被顺利推进。
- 调试完成后删除上述两条内联汇编，重新 `make qemu` 即可恢复正常的时钟节拍输出。
- 构建过程中遇到 `trapentry.S` 汇编错误：工具链报告未知 CSR `sbadaddr`。经查阅 RISC-V Privileged Spec 可知该 CSR 已更名为 `stval`，于是将 `csrr s3, sbadaddr` 改为 `csrr s3, stval`。改动后编译通过，并继续将 `stval` 的值写入 `trapframe.badvaddr`，供异常处理逻辑读取触发指令或故障地址。
